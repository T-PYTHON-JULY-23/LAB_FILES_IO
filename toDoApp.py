...
"""
user_input=input("do you want to add a new To-Do item? or enter exit to leave ".title()).lower()
file=open('to_do.txt','a+',encoding='utf-8')

while user_input!="exit":
    if user_input=='y':
        task=input("Enter your item :")
        file.write(task)
        file.write('\n')
    elif user_input=='n':
        user_input=input('do you want to list your To-Do items ? '.title()).lower()
        if user_input=='y':
            file.seek(0)
            content=file.read()
            print(content)
